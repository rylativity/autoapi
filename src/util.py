...
### TODO: Example of using automap_base to get tables and column types
# from sqlalchemy.ext.automap import automap_base
# engine = create_engine("postgresql+psycopg2://autoapi:autoapi@localhost:5432/autoapi")
# Base = automap_base()
# Base.prepare(autoload_with=engine)

# ## After calling .prepare(), Base.classes contains attributes for each table.  Extract the columns from the tables
# for table in Base.classes:
#     print(table)
#     print(table.__table__.columns.items())
###
...
class ModelEndpointFactory:
    ...
    def get_schemas(self, exclude: List[str] = None) -> List[str]:
        if "sqlite" in os.environ.get("DB_CONNECTION_STRING"):
